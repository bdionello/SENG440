...
# 38 "/usr/include/arm-linux-gnueabihf/bits/stdio.h" 3 4
...
# 127 "/usr/include/arm-linux-gnueabihf/bits/stdio.h" 3 4
extern __inline __attribute__ ((__gnu_inline__)) int
...
inline void filter_init(short int *X, short int *Y){
    ...
    X[0] = (short int)0x8001;
    X[1] = (short int)0x8001;
    for (i=2; i<100; i++)
        X[i] = (short int)0x7FFF;


    Y[0] = (short int)0xC000;
    Y[1] = (short int)0xC000;
}
...
void main(void){
    ...
    for (i=2; i<100; i+=4) {
        ...
        printf( "Y[%2d] = %+6hi = 0x%04hX ....... y[%2d] = %8.5f\n", i+1, Y[i+1], Y[i+1], i+1, ((float)Y[i+1])/16384 );
        printf( "Y[%2d] = %+6hi = 0x%04hX ....... y[%2d] = %8.5f\n", i+2, Y[i+2], Y[i+2], i+2, ((float)Y[i+2])/16384 );
        printf( "Y[%2d] = %+6hi = 0x%04hX ....... y[%2d] = %8.5f\n", i+3, Y[i+3], Y[i+3], i+3, ((float)Y[i+3])/16384 );
