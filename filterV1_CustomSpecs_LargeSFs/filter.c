...
void filter_init(short int *X, short int *Y){
    ...
    Y[0] = (short int)0x8001; //-32767 --> Normalized to y[0] = ~ -1 (-0.99997)
    Y[1] = (short int)0x8001; //-32767 --> Normalized to y[1] = ~ -1 (-0.99997)
}
...
void main(void){
    ...
    printf( "Y[ 0] = %+6hi = 0x%04hX ....... y[ 0] = %8.5f\n", Y[0], Y[0], ((float)Y[0])/16384 ); // Scale Factor = 2^14; used to be 2^15 = 32768
    printf( "Y[ 1] = %+6hi = 0x%04hX ....... y[ 1] = %8.5f\n", Y[1], Y[1], ((float)Y[1])/16384 ); // Scale Factor = 2^14; used to be 2^15 = 32768
    ...
    for (i=2; i<100; i++) {
        ...
        printf( "Y[%2d] = %+6hi = 0x%04hX ....... y[%2d] = %8.5f\n", i, Y[i], Y[i], i, ((float)Y[i])/16384 ); // Scale Factor = 2^14; used to be 2^15 = 32768
